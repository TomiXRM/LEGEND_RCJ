...
LineManage::LineManage(PinName LF,PinName LR,PinName LB,PinName LL) : lineF(LF),lineR(LR),lineB(LB),lineL(LL),serial(NC,NC){
        line_wait.start();
...
LineManage::LineManage(PinName LF,PinName LR,PinName LB,PinName LL,PinName tx,PinName rx) : lineF(LF),lineR(LR),lineB(LB),lineL(LL),serial(tx,rx){
        line_wait.start();
        lineOP.attach(mbed::callback(this,&LineManage::LineCheck),1.0 / 12000);
        serial.baud(460800);
        serial.attach(mbed::callback(this,&LineManage::serialLineGet));
        OutFlag = 0;
        OnWH1st = 0;
        OnWH2nd = 0;
        OnWH3rd = 0;
...
INLINE void LineManage::serialLineGet(){
        // linestate = Main.getc();
        front1 = bitRead(linestate,0);
        front2 = bitRead(linestate,1);
        right1 = bitRead(linestate,2);
        right2 = bitRead(linestate,3);
        back1 = bitRead(linestate,4);
        back2 = bitRead(linestate,5);
        left1 = bitRead(linestate,6);
        left2 = bitRead(linestate,7);
