...
INLINE void PositionMonitor(){
        ...
        while (ui.mode == POSITIONMONITOR) {
            //     cam.get();//NOTE:revで廃止
                ...
                pc.printf("OwnX:%d\tOwnY:%d\tOwnW:%d\tOpsX:%d\tOpsY:%d\tOpsW:%d\tOpsDir:%d\tOwnDir:%d\tOwnDist:%d\tOpsDist:%d\r\n",cam.OwnX,cam.OwnY,cam.OwnW,cam.OpsX,cam.OpsY,cam.OpsW,cam.calDegree(1),cam.calDegree(0),(uint16_t)sqrt(pow(cam.OwnX,2)+pow(cam.OwnY,2)),(uint16_t)sqrt(pow(cam.OpsX,2)+pow(cam.OpsY,2)));
                ui.putc('V');
