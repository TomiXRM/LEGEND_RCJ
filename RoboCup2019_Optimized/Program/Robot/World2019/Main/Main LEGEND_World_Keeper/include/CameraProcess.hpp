...
float CamModedir;
float CamModeTurndir;

PID CamModePidDir(&CamModedir, &CamModeTurndir, 0, 1.4, 0, 0.04,
                  DIRECT); //方向制御のPID);
...
void initCameraProcess() {
        ...
        CamModePidDir.SetMode(AUTOMATIC);
        CamModePidDir.SetOutputLimits(-70, 70);
        CamModePidDir.SetSampleTime(9); // 5
        posinum = 0;
}
...
INLINE void SetPosition() {
        ...
        while (ui.mode == SETPOSITION) {
                ...
                pc.printf("OwnX:%d\tOwnY:%d\tOpsX:%d\tOpsY:%d\tOpsDir:%d\tOwnDir:%d\r\n",
                          cam.OwnX, cam.OwnY, cam.OpsX, cam.OpsY, cam.calDegree(1),
                          cam.calDegree(0));
        }
        ui.attachRx(&ReceiveMode);
        pc.printf("OppositeRight   ownX:%d \townY:%d \topsX:%d \topsY:%d\r\n",
                  cam.PointOwnX[0], cam.PointOwnY[0], cam.PointOpsX[0],
                  cam.PointOpsY[0]);
        pc.printf("OppositeLeft    ownX:%d \townY:%d \topsX:%d \topsY:%d\r\n",
                  cam.PointOwnX[1], cam.PointOwnY[1], cam.PointOpsX[1],
                  cam.PointOpsY[1]);
        pc.printf("CenterPoint     ownX:%d \townY:%d \topsX:%d \topsY:%d\r\n",
                  cam.PointOwnX[2], cam.PointOwnY[2], cam.PointOpsX[2],
                  cam.PointOpsY[2]);
        pc.printf("OwnsideRight    ownX:%d \townY:%d \topsX:%d \topsY:%d\r\n",
                  cam.PointOwnX[3], cam.PointOwnY[3], cam.PointOpsX[3],
                  cam.PointOpsY[3]);
        pc.printf("OwnsideLeft     ownX:%d \townY:%d \topsX:%d \topsY:%d\r\n",
                  cam.PointOwnX[4], cam.PointOwnY[4], cam.PointOpsX[4],
                  cam.PointOpsY[4]);
        pc.printf("\r\n\r\n\r\n\r\n\r\n\r\n\r\n\r\n\r\n\r\n\r\n");
        wait(2);
}
...
INLINE void FacingGoal() {
        ...
        while (ui.mode == FACINGOPSGOAL) {
                ...
                CamModedir = cam.calDegree(1); //向いている角度の取得
                CamModePidDir.Compute();
                MD.run(-CamModeTurndir, -CamModeTurndir, -CamModeTurndir, -CamModeTurndir);
...
INLINE void PositionMonitor() {
        ...
        while (ui.mode == POSITIONMONITOR) {
                cam.get();
                wait_ms(10);
                // pc.printf("OwnX:%d \t OwnY:%d \t OpsX:%d \t
                ...
                pc.printf("OwnX:%d\tOwnY:%d\tOwnW:%d\tOwnDir:%d\tOwnRigh:%d\tOwnLeft:%"
                          "d\tOpsX:%d\tOpsW:%d\tOpsDir%d\r\n",
                          cam.OwnX, cam.OwnY, cam.OwnW, cam.calDegree(0),
                          cam.OwnGoalRightEnd, cam.OwnGoalLeftEnd, cam.OpsX, cam.OpsW,
                          cam.calDegree(1));
                ui.putc('V');
