...
// NOTE 11/16
// ComSLPはなぜか使えません。とりあえず今はcom_slp無視してください。11/16 By
// 祐聖 NOTE 12/3  ComSLP使えちゃいましたあはは
//[参照]2018/11/15 モータまわすぜい @ゆうせいのレポート(と言う名のなにか).pages

//------------------------------------------------------------------------------------
Motors::Motors(PinName _pwm1, PinName _pwm2, PinName _pwm3, PinName _pwm4,
               PinName _comSleep)
        : m1(_pwm1), m2(_pwm2), m3(_pwm3), m4(_pwm4), slp(_comSleep) {
        period_us(30); // 50
        stopAll();
        Deg_m1 = 30;
        Deg_m2 = 150;
