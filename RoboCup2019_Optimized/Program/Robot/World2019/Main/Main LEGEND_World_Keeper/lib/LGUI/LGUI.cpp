...
LGUI::LGUI(RawSerial *_pc, DigitalOut *_slp, PinName tx, PinName rx)
        : device(tx, rx) {
        pc = _pc;
        slp = _slp;
        ...
        Entry(MODESW, callback(this, &LGUI::Before_ModeSW),
              callback(this, &LGUI::Body_ModeSW), DONOTHING);
}
...
/*LGUI::Entry.
   ...
   If you don't need to do something in 2nd to 4th argument,you can put there
   DONOTHING.
 */

void LGUI::Entry(char c, Callback<void()> before, Callback<void()> loop,
                 Callback<void()> after) {
...
unsigned char LGUI::findIndex(char index) {
        ...
        unsigned char ArrayLength =
                (sizeof(modeList) / sizeof(modeList[0])); // Set "How many modes?""
        unsigned char i = 0;
...
void LGUI::Run() {
        ...
        if (i_mode == 255) {
                ...
                pc->printf(
                        "[ERROR]{LGUI} Unknown mode character. '%c'.\n Switch for modesw.",
                        mode); //とりまprint
                (After[findIndex(
                               modeLast)])(); //前の生きてた頃の片付け(=前のモードの遺品整理)
                modeLast =
                        MODESW; //モードを強制的に修正(生きてたモードの影を消してクラッシュの痕跡をなくす)
