...
extern OpenMV cam;
extern Motors MD;
extern AnalogIn PSB_B;

float CamModedir;
float CamModeTurndir;

PID CamModePidDir(&CamModedir, &CamModeTurndir,0, 1.4, 0, 0.04,DIRECT);//方向制御のPID);
...
INLINE void PositionMonitor(){
        ...
        while (ui.mode == POSITIONMONITOR) {
                ...
                pc.printf("OwnX:%d\tOwnY:%d\tOwnW:%d\tOwnDir:%d\tOwnRigh:%d\tOwnLeft:%d\tBack:%d\tOpsX:%d\tOpsW:%d\tOpsDir%d\r\n",cam.OwnX,cam.OwnY,cam.OwnW,cam.calDegree(0),cam.OwnGoalRightEnd,cam.OwnGoalLeftEnd, (uint16_t)(PSB_B.read()*260),cam.OpsX,cam.OpsW,cam.calDegree(1));
                ui.putc('V');
