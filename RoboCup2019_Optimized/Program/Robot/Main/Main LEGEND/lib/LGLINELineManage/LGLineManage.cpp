...
void LineManage::LineCheck(){
        ...
        if(front== 1 || right == 1 || back== 1 || left == 1) {
                if( OnWH1st == 0) {//初回
                        ...
                        LastWhiteSensor = 10-OnWH1st-OnWH2nd-OnWH3rd;
                }else{
                        //踏んでない
                }
                OnFlag = 1;
...
void LineManage::SetLine(VectorXY *vector,VectorXY *IRv,VectorXY *Bound){
        if(OnFlag == 1) {
                ...
                if(FirstWhiteSensor == 1 && LastWhiteSensor == 3 && Raw != 0) {
                        vector->y = -90;
                        OutFlag = 1;
                }else if(FirstWhiteSensor == 2 && LastWhiteSensor == 4 && Raw != -90) {
                        vector->x = -90;
                        OutFlag = 1;
                }else if(FirstWhiteSensor == 3 && LastWhiteSensor == 1 && Raw != 180) {
                        vector->y = 90;
                        OutFlag = 1;
                }else if(FirstWhiteSensor == 4 && LastWhiteSensor == 2 && Raw != 90) {
                        vector->x = 90;
                        OutFlag = 1;
                }else{
                        OutFlag = 0;
...
void LineManage::addHistory(short order_){
...
bool LineManage::SearchHistory(short query_order){

        bool ret=false;

        short i=0;
        for(i=0; i<100; i++) {
                if(history[i]==query_order) ret=true;
        }
