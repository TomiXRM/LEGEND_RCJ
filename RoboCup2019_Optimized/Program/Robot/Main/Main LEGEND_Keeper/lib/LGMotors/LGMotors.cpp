...
//------------------------------------------------------------------------------------
Motors::Motors(PinName _pwm1,PinName _pwm2,PinName _pwm3,PinName _comSleep) : m1_(_pwm1),m2_(_pwm2),m3_(_pwm3),slp(_comSleep),m1(_pwm1),m2(_pwm2),m3(_pwm3){
        period_us(24);//50
        m1_.mode(PullUp);
        m2_.mode(PullUp);
        m3_.mode(PullUp);
...
void Motors::run(float p1,float p2,float p3){
...
int Motors::drive(int deg,float power,float degree){
        ...
        p1 = (sin((deg-60) * PI / 180) * power+degree);
        p2 = (sin((deg+60) * PI / 180) * power+degree);
        p3 = (sin((deg+180) * PI / 180) * power+degree);
