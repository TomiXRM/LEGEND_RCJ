...
volatile int32_t IR_PulseWidth[16]; //パルス幅16個分
volatile int32_t IR_StartTime[16]; //パルス幅16個分
volatile int32_t IR_Count[16];
volatile bool IR_ActiveSensor[16];
volatile uint8_t IR_count_counter[16];
...
INLINE void SendToMaster(){
        led = 1;
        Nucleo.putc('J'); wait_us(5);
        Nucleo.putc(data % 256); wait_us(5);
        Nucleo.putc(data / 256); wait_us(5);
        Nucleo.putc(dist); 
}
...
void setup(){
        ...
        BaseIRTimer.start();
        vector_calF = 1;
        // IRPrint.attach_us(&SendToMaster,2000);
        IR1.fall(&IR00change);
        IR2.fall(&IR01change);
        IR3.fall(&IR02change);
        IR4.fall(&IR03change);
