...
size_t Adafruit_GFX::writeChar(uint8_t c)
{
        ...
        else
        {
                ...
                cursor_x += (textsize*6.0);
                if (wrap && (cursor_x > (_width - (textsize*6.0))))
                {
                        cursor_y += (textsize*8.0);
                        cursor_x = 0;
                }
...
// draw a character
void Adafruit_GFX::drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, float size)
{
        if(
                (x >= _width) || // Clip right
