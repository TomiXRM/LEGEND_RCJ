...
void before_Main(){
        moveBackTimer.start(); moveBackTimer.reset();
        keepCamDirT.start(); keepCamDirT.reset();
        dribHoldBackT.start(); dribHoldBackT.reset();
        dribStackT.start(); dribStackT.reset();
        dribBallHoldT.start(); dribBallHoldT.reset();
        dribHoldOwnSide.start(); dribHoldOwnSide.reset();
        missTheBallT.start(); missTheBallT.reset();
}
...
//カメラで幅と端を書き込むプログラム
        ...
        MouseV.x = mouse.x; MouseV.y = mouse.y;
                ...
                if(diffDeg < 35) {
                        ...
                        if(dribBallHoldT.read_ms() < 70) {
                                ...
                                if(abs(camera.OpsDir) < 50) {
                                        useOpsGoalDir = 2;
                                        DriveV.y = 40; DriveV.x = 1.2*camera.OpsX;
                                }else{
                                        driblOrder = 3;
                                        useOpsGoalDir = 1;
                                        DriveV.x = camera.OpsX * 4;
...
INLINE void BallTracking(uint8_t MovingSpeed){
        ...
        if(abs(sensors.dir) > 70) {useOpsGoalDir = 0;}
